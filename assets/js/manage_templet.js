const insertTempletBtn = document.getElementById('insert_templet_btn');
const imgFile = document.getElementById('img_file');
const searchTag = document.getElementById('search_tag');
const insertTempletform = document.getElementById('insert_templet_form');

insertTempletBtn.addEventListener('click', function () {
    if (confirm("등록하시겠습니까?")) {
        if (!imgFile.value) {
            alert('이미지를 등록해주세요.');
            imgFile.focus();
